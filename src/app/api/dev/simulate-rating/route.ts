...
export async function GET() {
  try {
    ...
    if (!renterUser) {
      ...
      renterUser = await prisma.user.create({
        data: {
          ...
          email: 'testrenter_idflow_' + Date.now() + '@wardrob.com',
          role: 'RENTER',
    ...
    if (!booking) {
      console.log('No COMPLETED bookings found. Cannot simulate rating.');
      return NextResponse.json({ success: true, message: 'ID flow complete, but no completed booking found for rating.' });
    }
    ...
    const avg = listerReviews.reduce((sum: number, r: any) => sum + Number(r.rating), 0) / listerReviews.length;
    await prisma.user.update({
    ...
    const rAvg = renterReviews.reduce((sum: number, r: any) => sum + Number(r.rating), 0) / renterReviews.length;
    await prisma.user.update({
      where: { id: renterId },
      data: { rating: rAvg }
    });
